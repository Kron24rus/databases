...
insert into storagedb.product_dim (ProductID, name, update_date, price, buy_price)
select p.ProductID, p.name, p.update_date, p.price, p.buy_price from shopdb.product p
...
insert into storagedb.sales_fact (product_dim_productID, provider_dim_providerID, category_dim_categoryID, order_dim_orderID, quantity, sell_price, product_price, shop_dim_shopID, date_dim_dateID, order_productID, discount)
(select p.ProductID, pr.ProviderID, cat.CategoryID, ord.OrderID, op.quantity, op.sell_price, p.price, 1, dd.DateID, op.OrderProductID, op.discount
